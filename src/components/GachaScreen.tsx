import { Dices, Sparkles, Swords, SkipForward } from 'lucide-react';
...
export default function GachaScreen({ gachaNameDisplay, isSpinning, hasSpun, putarGacha, masukKeSoalGiliran, lewatiGiliran, resetGame }: Props) {
  return (
      ...
      <div className="bg-white/95 p-6 md:p-10 rounded-[2rem] shadow-2xl w-full max-w-lg text-center border border-white/60 relative overflow-hidden">
        ...
        <div className="relative z-10 flex justify-center my-6 md:my-8">
          <div className="relative w-64 md:w-72">
            <div className="bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 p-[3px] rounded-[1.4rem] shadow-2xl">
              <div className={`relative bg-gradient-to-b from-white to-indigo-100 rounded-[1.2rem] overflow-hidden aspect-[63/88] flex flex-col ${isSpinning ? 'anim-rolling' : 'anim-pop-up'}`}>
                <div className="h-8 md:h-10 w-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-between px-3 md:px-4 z-10 relative">
                  ...
                  <span className="text-white font-black text-[8px] md:text-[10px] uppercase tracking-widest drop-shadow">
                    {isSpinning ? 'Undi...' : 'Giliran'}
                  </span>
                </div>

                <div className="flex-1 flex flex-col items-center justify-center px-4 relative z-10 gap-3">
                  <Sparkles className={`w-6 h-6 md:w-8 md:h-8 ${isSpinning ? 'text-slate-300' : 'text-blue-400'} animate-bounce-slow`} strokeWidth={2} />
                  <p className={`font-black tracking-tighter break-words w-full px-2 leading-tight ${isSpinning ? 'text-4xl md:text-5xl text-slate-300' : 'text-5xl md:text-6xl text-blue-700 drop-shadow-sm'}`}>
        ...
        <div className="relative z-10 mt-6 md:mt-8">
          {!hasSpun ? (
            ...
            <>
              ...
              {!isSpinning && (
                <button
                  onClick={lewatiGiliran}
                  className="mt-2 w-full flex items-center justify-center gap-1.5 text-slate-400 font-bold py-2 text-sm hover:text-slate-600 relative z-10"
                >
        ...
        <button
          onClick={resetGame}
          className="mt-2 w-full text-slate-400 font-bold py-2 text-sm hover:text-slate-600 relative z-10"
        >
          Batalkan & Kembali
        </button>
      </div>

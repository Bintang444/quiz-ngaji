    ...
    <div className="flex flex-col min-h-screen w-full max-w-2xl mx-auto p-4 relative z-10">
      <div className="flex justify-between items-center bg-white/95 p-5 rounded-3xl shadow-lg mb-8 sticky top-4 z-20 backdrop-blur-md border-4 border-slate-100">
        <div className="flex-1">
          <p className="text-xs text-slate-400 font-black mb-2 tracking-widest uppercase">Pertanyaan {currentProgress} / {totalProgress}</p>
        ...
        <div className="flex-1 text-right ml-4 border-l-4 pl-4 border-slate-100">
          <p className="text-xs text-slate-400 font-black uppercase tracking-widest mb-1">{mode === 'giliran' ? 'Giliran:' : 'Skor Kelas:'}</p>
          <p className={`text-2xl font-black truncate leading-none ${mode === 'giliran' ? 'text-orange-600' : 'text-purple-600'}`}>{namaGiliran}</p>
          {mode === 'giliran' ? (
            <span className="mt-2 text-sm bg-yellow-100 text-yellow-700 px-3 py-1 rounded-xl font-black border-2 border-yellow-200">{skorIndividu[namaGiliran] || 0} BINTANG</span>
          ) : (
            <span className="mt-2 text-2xl font-black text-amber-500 drop-shadow-sm flex items-center gap-1 justify-end">
              {skorKolektif}
              <Star className="w-7 h-7" fill="currentColor" strokeWidth={1.5} />
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-start pb-8">
        <div className="relative mt-12 mb-6">
          ...
          <div className="bg-white p-8 pt-20 pb-10 rounded-[3rem] shadow-xl border-b-[10px] border-blue-200 text-center relative z-0">
            <h2 className="text-3xl md:text-4xl font-black text-slate-800 leading-snug drop-shadow-sm">{soalSekarang.pertanyaan}</h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
          {soalSekarang.pilihan.map((pilihan, idx) => (
            <button
              ...
              className={`w-full text-xl md:text-2xl font-black py-7 px-5 rounded-[2rem] border-4 transition-all duration-200 disabled:cursor-not-allowed ${buttonState(pilihan)}`}
            >
              {pilihan}
            </button>
          ))}
        ...
        {jawabanDipilih !== null && (
          <div className="fixed inset-0 flex items-center justify-center pointer-events-none z-50">
            <div className={`bg-white/95 backdrop-blur-sm px-14 py-10 rounded-[4rem] shadow-2xl border-[12px] transform transition-transform ${feedbackAnim} ${isBenar ? 'border-emerald-400 rotate-2' : 'border-amber-400 -rotate-2'}`}>
              <div className="flex items-center justify-center gap-4 mb-2">
                {isBenar ? (
                  <PartyPopper className="w-16 h-16 text-emerald-500" strokeWidth={1.5} />
                ) : (
                  <Lightbulb className="w-16 h-16 text-amber-500" strokeWidth={1.5} />
                )}
                <p className="text-6xl md:text-7xl font-black text-center drop-shadow-md">
                  {isBenar ? 'BENAR!' : 'BELUM TEPAT!'}
                </p>
              </div>
              {isBenar ? (
                <p className="text-center text-4xl font-black mt-2 text-amber-500 bg-amber-50 rounded-full py-2 border-4 border-amber-200 flex items-center justify-center gap-2">
                  +1 Bintang! <Star className="w-9 h-9" fill="currentColor" strokeWidth={1.5} />
                </p>
              ) : (
                <div className="mt-2 text-center">
                  <p className="text-lg font-bold text-amber-500">Nggak apa-apa, yuk ingat lagi!</p>
                  <div className="mt-3 bg-rose-50 rounded-3xl border-4 border-rose-200 p-5">
                    <p className="text-sm font-black uppercase tracking-widest text-rose-400 mb-1">Jawaban yang benar adalah</p>
                    <p className="text-3xl md:text-4xl font-black text-emerald-600">{soalSekarang.jawabanBenar}</p>
                  </div>
                  <button
                    onClick={lanjutSoal}
                    className="pointer-events-auto mt-5 w-full py-4 bg-emerald-500 text-white font-black text-2xl rounded-2xl shadow-[0_8px_0_rgb(4,120,87)] hover:-translate-y-1 active:shadow-none active:translate-y-2 transition-all"
                  >
                    Lanjut Yuk!
                  </button>
                </div>
